  ...
  render() 
  {
    ...
    return (
      <View style={styles.container}>
        ...
        <FlatList data={categories} renderItem={({ item }) => <CategoryListItem  category={item} onPress={() => navigation.navigate('Category')}/>} keyExtractor={(item) => `${item.id}`}  style={styles.containerScrollView} contentContainerStyle={{backgroundColor: '#DDD', padding: 20}} />
      </View>
    )
  }
}
